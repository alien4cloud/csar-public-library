...
function install_rpackages {
   ...
   packages=(ade4 arules arulesViz biglm boot C50 car caret combinat corrplot dplyr doSNOW e1071
             FactoMineR ff ffbase foreach foreign gbm gplots ggplot2 glmnet gmodels grplasso ipred
             kernlab lattice leaps LiblineaR lubridate MASS mice missForest nnet penalized plsRglm
             plyr pROC questionr randomForest randtoolbox network RColorBrewer ROCR rpart rpart.plot
             Rserve sas7bdat shiny snow speedglm stringr tidyr tm tree ggvis)
