...
resource "openstack_networking_port_v2" "a4c_internet_port" {
  ...
  admin_state_up = "true"
  security_group_ids = "${var.EXTERNAL_SECGROUP_IDS}"
}
...
resource "openstack_compute_floatingip_v2" "a4c_fip" {
  pool = "${var.POOL_NAME}"
}

resource "openstack_compute_floatingip_associate_v2" "a4c_floating_ip" {
  floating_ip = "${openstack_compute_floatingip_v2.a4c_fip.address}"
...
output "PUBLIC_IP_ADDRESS" {
  value = "${openstack_compute_floatingip_v2.a4c_fip.address}"
}
